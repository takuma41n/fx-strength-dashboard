...
| 柱 | ウェイト | 中身 | 状態 |
|---|---|---|---|
| ① 金利（10年） | 40% | 10年国債利回りの Δ5日 / Δ20日 / Δ60日 | 実装済み（日次） |
...
| 米10年金利・HYスプレッド・期待インフレ・ECB政策金利 | FRED（`fredgraph.csv`） |
| ユーロ圏10年AAAスポット（日次） | ECB Data Portal |
| 日本10年国債（日次・和暦・Shift-JIS） | 財務省 `jgbcm_all.csv` + `jgbcm.csv` |
| 英10年スポット・英OIS 1年（日次） | BOE イールドカーブ zip |
| 豪10年国債（日次） | RBA `f2-data.csv` |
| スイス連邦債10年（日次） | SNB `www.snb.ch/public/rates/interestRates`（過去分は停止済みキューブ `rendoblid`） |
...
`bootstrap_history.py` は BOE の全履歴アーカイブ（約39MB）から英10年金利の過去分を一度だけ
